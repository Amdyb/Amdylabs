  ...
  return (
    <>
      <nav style={{
        ...
        padding: '0 24px',
        background: scrolled ? 'rgba(4,8,26,0.96)' : 'transparent',
        backdropFilter: scrolled ? 'blur(24px)' : 'none',
        borderBottom: scrolled ? '1px solid rgba(30,45,74,0.5)' : 'none',
        transition: 'all 0.3s',
      }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 72 }}>
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <Link href="/contact" style={{
              background: 'linear-gradient(135deg,#1a6ef5,#0050c8)', color: '#fff',
              padding: '10px 22px', borderRadius: 8, textDecoration: 'none',
              fontSize: 13, fontWeight: 700, boxShadow: '0 4px 20px rgba(26,110,245,0.3)',
            }} className="nav-cta">Start a Project →</Link>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, background: '#04081a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 28 }}>
          ...
          {links.map(l => (
            <Link key={l.href} href={l.href} onClick={() => setMobileOpen(false)}
              style={{ fontFamily: 'Syne, system-ui', fontSize: 28, fontWeight: 700, color: '#fff', textDecoration: 'none' }}>
              {l.label}
            </Link>
          ))}
          <Link href="/contact" onClick={() => setMobileOpen(false)}
            style={{ background: 'linear-gradient(135deg,#1a6ef5,#0050c8)', color: '#fff', padding: '14px 40px', borderRadius: 10, textDecoration: 'none', fontWeight: 700, fontSize: 16, marginTop: 8, boxShadow: '0 8px 32px rgba(26,110,245,0.3)' }}>
            Start a Project →
          </Link>
        </div>
      )}
